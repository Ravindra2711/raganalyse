...
claude_4_5_sonnet_latest = None 

# Anthropic models
claude_3_5_haiku_latest = None
claude_3_7_sonnet_latest = None
claude_3_5_sonnet_latest = None
claude_sonnet_4_20250514 = None
claude_3_opus_latest = None
claude_opus_4_20250514 = None
claude_opus_4_1 = None

# Groq models
deepseek_r1_distill_llama_70b = None
gpt_oss_120B = None
gpt_oss_20B = None

gemini_2_5_pro_preview_03_25 = None
...
def get_model_object(model_name: str):
    """Get model object by name (may be None if DSPy disabled)."""
    return MODEL_OBJECTS.get(model_name)
...
"""Unified completion wrapper for all providers via LiteLLM (supports Gemini, OpenAI, Anthropic, Groq)."""
def completion_call(model: str, messages, temperature: float, max_tokens: int, stream: bool = False):
    try:
        import litellm
        ...
        if stream:
            raise NotImplementedError("streaming not supported in completion_call")
        # LiteLLM returns an OpenAI-compatible response
        choice = resp.choices[0]
        # content may be at choice.message["content"] or choice["text"]
        content = None
        ...
        usage = {
            "prompt_tokens": getattr(resp, "usage", {}).get("prompt_tokens", 0) if hasattr(resp, "usage") else 0,
            "completion_tokens": getattr(resp, "usage", {}).get("completion_tokens", 0) if hasattr(resp, "usage") else 0,
            "total_tokens": getattr(resp, "usage", {}).get("total_tokens", 0) if hasattr(resp, "usage") else 0,
        }
        return {"content": content or "", "usage": usage}
    except Exception:
        # Fallback to dspy if LiteLLM path fails; keeps server usable
        lm = get_model_object(model)
        prompt = "\n".join([m.get("content", "") for m in messages])
        text = lm(prompt)
        return {"content": str(text), "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}}
